  ...
  return (
    <div className="space-y-3">
      {showHistory ? (
        ...
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-surface-200 bg-white">
          <span className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 bg-brand-100 text-brand-700">
            <FileText className="w-4 h-4" />
          </span>
          <span className="min-w-0 flex-1">
            <span className="text-sm font-semibold text-gray-900 truncate block">{selected.file_name}</span>
            <span className="block text-xs text-gray-500 mt-0.5">
              {format(parseISO(selected.created_at), 'd MMM yyyy')}
            </span>
          </span>
          <a
